-- [[ SYAAA HUB V7.6 - PURPLE THEME + 3D IMAGE ADVANCED CONTROLS ]] --
...
local function runSyaaHub()
    ...
    task.spawn(function()
        pcall(function() playerIcon.Image = Players:GetUserThumbnailAsync(localPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size150x150) end)
    end)
    ...
    makeSidebarIcon("112703342701931", 96, "Tools", UDim2.new(0, 30, 0, 30)) -- ICON BARU BROO
    ...
    infJumpRow.MouseButton1Click:Connect(function()
        isInfJump = not isInfJump
        setInfJump(isInfJump)
    end)
    ...
    UserInputService.InputChanged:Connect(function(i)
        if wsSld and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
            local pos = math.clamp((i.Position.X-wsBg.AbsolutePosition.X)/wsBg.AbsoluteSize.X,0,1)
            wsFill.Size = UDim2.new(pos,0,1,0); wsKnob.Position = UDim2.new(pos,-7,0.5,-7)
            wsVal = math.floor(pos*200)
            if wsVal < 16 then wsVal = 16 end
            wsLab.Text = "Speed: "..wsVal
        end
    end)
    local isWsEnabled = false
    wsRow.MouseButton1Click:Connect(function() isWsEnabled = not isWsEnabled; setWsState(isWsEnabled) end)
    RunService.Heartbeat:Connect(function()
        if isWsEnabled and localPlayer.Character and localPlayer.Character:FindFirstChild("Humanoid") then
            localPlayer.Character.Humanoid.WalkSpeed = wsVal
        end
    end)
    ...
    tpFrame.Size = UDim2.new(0.92,0,0,80); tpFrame.Position = UDim2.new(0.04,0,0,tY)
    tpFrame.BackgroundColor3 = Color3.fromRGB(10,5,15); tpFrame.BackgroundTransparency = 0.5; tpFrame.Parent = pTools
    tpFrame.ScrollBarThickness = 2; tpFrame.ScrollBarImageColor3 = Color3.fromRGB(150,40,255)
    tpFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y; tpFrame.CanvasSize = UDim2.new(0,0,0,0); tpFrame.ScrollingDirection = Enum.ScrollingDirection.Y
    Instance.new("UICorner",tpFrame).CornerRadius = UDim.new(0,6); Instance.new("UIStroke",tpFrame).Color = Color3.fromRGB(80,30,120)
    local tpLayout = Instance.new("UIListLayout",tpFrame); tpLayout.Padding = UDim.new(0,2); tpLayout.SortOrder = Enum.SortOrder.Name
    local tpPad = Instance.new("UIPadding",tpFrame); tpPad.PaddingTop = UDim.new(0,2); tpPad.PaddingLeft = UDim.new(0,2); tpPad.PaddingRight = UDim.new(0,2); tpPad.PaddingBottom = UDim.new(0,2)
    tY = tY+90
    ...
    local function refreshTpList()
        ...
        if #list <= 1 then
            local el = Instance.new("TextLabel"); el.Size = UDim2.new(1,0,0,24); el.BackgroundTransparency = 1; el.Text = "Cuma lu doang di server 🗿"; el.TextColor3 = Color3.fromRGB(100,100,100); el.Font = Enum.Font.Gotham; el.TextSize = 10; el.Parent = tpFrame; table.insert(tpRows,el); return
        end
        for _,p in ipairs(list) do
            if p ~= localPlayer then
                local row = Instance.new("TextButton"); row.Size = UDim2.new(1,0,0,30); row.BackgroundColor3 = Color3.fromRGB(130,40,255); row.BackgroundTransparency = 0.6; row.Text = ""; row.AutoButtonColor = false; row.Parent = tpFrame; Instance.new("UICorner",row).CornerRadius = UDim.new(0,4)
                
                local ava = Instance.new("ImageLabel"); ava.Size = UDim2.new(0, 24, 0, 24); ava.Position = UDim2.new(0, 4, 0.5, -12); ava.BackgroundTransparency = 1; ava.Parent = row; Instance.new("UICorner",ava).CornerRadius = UDim.new(1,0)
    ...
    tpRefresh.MouseButton1Click:Connect(refreshTpList)
    task.spawn(function() task.wait(1) refreshTpList() end)
    ...
    imgInput.Size = UDim2.new(0.92, 0, 0, 30); imgInput.Position = UDim2.new(0.04, 0, 0, tY); imgInput.BackgroundColor3 = Color3.fromRGB(20, 10, 30); imgInput.TextColor3 = Color3.fromRGB(255, 255, 255); imgInput.PlaceholderText = "Masukkan ID Gambar..."
    imgInput.Font = Enum.Font.GothamBold; imgInput.TextSize = 11; imgInput.Parent = pTools
    Instance.new("UICorner", imgInput).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", imgInput).Color = Color3.fromRGB(130,40,255)
    tY = tY + 36
    ...
    imgListFrame.Size = UDim2.new(0.92,0,0,70); imgListFrame.Position = UDim2.new(0.04,0,0,tY)
    imgListFrame.BackgroundColor3 = Color3.fromRGB(10,5,15); imgListFrame.BackgroundTransparency = 0.5; imgListFrame.Parent = pTools
    imgListFrame.ScrollBarThickness = 2; imgListFrame.ScrollBarImageColor3 = Color3.fromRGB(150,40,255)
    imgListFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y; imgListFrame.CanvasSize = UDim2.new(0,0,0,0); imgListFrame.ScrollingDirection = Enum.ScrollingDirection.Y
    Instance.new("UICorner",imgListFrame).CornerRadius = UDim.new(0,6); Instance.new("UIStroke",imgListFrame).Color = Color3.fromRGB(80,30,120)
    local ilLayout = Instance.new("UIListLayout",imgListFrame); ilLayout.Padding = UDim.new(0,2)
    local ilPad = Instance.new("UIPadding",imgListFrame); ilPad.PaddingTop = UDim.new(0,2); ilPad.PaddingLeft = UDim.new(0,2); ilPad.PaddingRight = UDim.new(0,2); ilPad.PaddingBottom = UDim.new(0,2)
    tY = tY + 80
    ...
    -- KONTROL PUTAR ATAS / BAWAH BARU DITAMBAHIN DI SINI
    ...
    local function refreshImgList()
        for _,r in pairs(listRows) do pcall(function() r:Destroy() end) end; listRows = {}
        for i, data in ipairs(spawnedImages) do
            local isAct = (activeImage == data.part)
            local row = Instance.new("TextButton"); row.Size = UDim2.new(1,0,0,24); row.BackgroundColor3 = isAct and Color3.fromRGB(200,80,255) or Color3.fromRGB(80,30,120); row.BackgroundTransparency = 0.5; row.Text = (isAct and "◉ " or "○ ")..data.name; row.TextColor3 = Color3.fromRGB(255,255,255); row.Font = Enum.Font.GothamBold; row.TextSize = 10; row.Parent = imgListFrame; Instance.new("UICorner",row).CornerRadius = UDim.new(0,4)
            row.MouseButton1Click:Connect(function()
                activeImage = data.part
                refreshImgList()
            end)
    ...
    spwBtn.MouseButton1Click:Connect(function()
        ...
        p.Name = "Syaa3DImage_"..imgCount
        p.Size = Vector3.new(6, 6, 0.1) 
        p.Anchored = true
        p.CanCollide = false
        p.Massless = true
        ...
        local frontCFrame = hrp.CFrame * CFrame.new(0, 0, -5) 
        p.CFrame = CFrame.lookAt(frontCFrame.Position, hrp.Position)
        
        local realUrl = "rbxthumb://type=Asset&id="..idText.."&w=420&h=420"
        ...
        table.insert(spawnedImages, {id = idText, part = p, name = "Gambar ID: "..string.sub(idText, 1, 5)..".."})
        activeImage = p
        refreshImgList()
    end)
    ...
    clrBtn.MouseButton1Click:Connect(function()
        if activeImage then 
            activeImage:Destroy()
            for i, data in ipairs(spawnedImages) do
                if data.part == activeImage then table.remove(spawnedImages, i); break end
            end
            activeImage = nil
            refreshImgList()
        end
    ...
    btnScaleUp.MouseButton1Click:Connect(function()
        if activeImage then activeImage.Size = activeImage.Size + Vector3.new(0.5, 0.5, 0) end
    end)
    btnScaleDown.MouseButton1Click:Connect(function()
        if activeImage then activeImage.Size = Vector3.new(math.max(0.5, activeImage.Size.X - 0.5), math.max(0.5, activeImage.Size.Y - 0.5), 0.1) end
    end)

    local stepPos = 0.5
    local stepRot = math.rad(15)
    local function mod3D(cf) if activeImage then activeImage.CFrame = activeImage.CFrame * cf end end
    
    btnLeft.MouseButton1Click:Connect(function() mod3D(CFrame.new(stepPos, 0, 0)) end) 
    btnRight.MouseButton1Click:Connect(function() mod3D(CFrame.new(-stepPos, 0, 0)) end) 
    btnUp.MouseButton1Click:Connect(function() mod3D(CFrame.new(0, stepPos, 0)) end)
    btnDown.MouseButton1Click:Connect(function() mod3D(CFrame.new(0, -stepPos, 0)) end)
    btnFwd.MouseButton1Click:Connect(function() mod3D(CFrame.new(0, 0, stepPos)) end)
    btnBack.MouseButton1Click:Connect(function() mod3D(CFrame.new(0, 0, -stepPos)) end)
    
    -- LOGIKA ROTASI (X, Y, Z)
    btnRotL.MouseButton1Click:Connect(function() mod3D(CFrame.Angles(0, stepRot, 0)) end)
    btnRotR.MouseButton1Click:Connect(function() mod3D(CFrame.Angles(0, -stepRot, 0)) end)
    btnTiltL.MouseButton1Click:Connect(function() mod3D(CFrame.Angles(0, 0, stepRot)) end)
    btnTiltR.MouseButton1Click:Connect(function() mod3D(CFrame.Angles(0, 0, -stepRot)) end)
    btnRotUp.MouseButton1Click:Connect(function() mod3D(CFrame.Angles(stepRot, 0, 0)) end)
    btnRotDown.MouseButton1Click:Connect(function() mod3D(CFrame.Angles(-stepRot, 0, 0)) end)
